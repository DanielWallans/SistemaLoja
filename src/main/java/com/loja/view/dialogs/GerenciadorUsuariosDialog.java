...
public class GerenciadorUsuariosDialog extends JDialog {
    ...
    private void initComponents() {
        ...
        JButton btnEditar = new JButton("✏️ Editar Usuário");
    ...
    private void alterarSenhaSelecionado() {
        ...
        int opt = JOptionPane.showConfirmDialog(this, new Object[]{"Digite a nova senha para " + u.getNome() + ":", pwd}, "Redefinir Senha", JOptionPane.OK_CANCEL_OPTION);
        if (opt == JOptionPane.OK_OPTION) {
            String novaSenha = new String(pwd.getPassword());
            if (novaSenha.trim().isEmpty()) {
                JOptionPane.showMessageDialog(this, "A senha não pode ficar vazia!", "Aviso", JOptionPane.WARNING_MESSAGE);
                return;
            }
            boolean ok = usuarioDAO.alterarSenha(u.getId(), novaSenha);
            if (ok) {
                JOptionPane.showMessageDialog(this, "Senha do usuário #" + u.getId() + " alterada com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
            }
        }
    }
